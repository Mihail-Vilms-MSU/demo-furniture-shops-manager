...
AS $$
  ...
  DROP TABLE IF EXISTS product_to_purchase CASCADE;
...
AS $$
  BEGIN
    BEGIN
        FOR i IN 1001..1008 LOOP /* #TODO 120 - Magic number */
          FOR j IN 1001..1015 LOOP /* #TODO 120 - Magic number */
...
AS $$
  BEGIN
    BEGIN
      FOR i IN 1001..(1001 + $1) LOOP
        DECLARE
          rand_employee_id INTEGER := 1000 + floor(random() * 120 + 1)::int; /* #TODO 120 - Magic number - count of employees */

        BEGIN
          INSERT INTO purchase (id, employee_id, shop_id, registered_at)
          SELECT i, rand_employee_id, e.shop_id,
          timestamp '2019-03-01 00:00:00' + random() * (timestamp '2019-03-31 23:59:59' - timestamp '2019-03-01 00:00:00')
          FROM employee e
          WHERE e.id = rand_employee_id;
        END;
...
BEGIN
  ...
  CALL fill_demo_data_purchase(2000);
  COMMIT;
END;
$$;
